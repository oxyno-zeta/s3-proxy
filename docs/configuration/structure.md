...
| userIsolation                            | Boolean                                                                                                                                      | No       | `false`  | When enabled, the proxy transparently prefixes every S3 key with the authenticated `<username>/`. Users never see their own username in the URL: a request for `/file.txt` is routed to `<bucketPrefix>/<username>/file.txt`. Listings expose only the user's own folder with the username hidden from displayed paths. Applies to GET, HEAD, PUT and DELETE. Requires an authenticated user; requests without one are rejected with 403. See [User Isolation](../feature-guide/user-isolation.md). |
| userIsolationAdmins                      | [String]                                                                                                                                     | No       | `nil`    | List of usernames that bypass the injection and can access the whole bucket prefix as if isolation were off. Only effective when `userIsolation` is enabled.                                                                                                                                       |
