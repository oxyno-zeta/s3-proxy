...
| userIsolation                            | Boolean                                                                                                                                      | No       | `false`  | When enabled, each authenticated user can only access files under their own username-prefixed folder (e.g., `/<username>/`). This applies to GET, PUT, and DELETE operations. Directory listings at the root level are filtered to show only the user's own entries.                 |
| userIsolationAdmins                      | [String]                                                                                                                                     | No       | `nil`    | List of usernames that bypass user isolation and can access all folders. Only effective when `userIsolation` is enabled.                                                                                                                                                            |
