...
| uidClaim      | String                                              | No       | `""`             | Claim used to build OIDC user identifier (`GenericUser.GetIdentifier()`). If not configured, it default to `preferred_username`, then falls back to `email`                                                           |
